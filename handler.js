...
—◉ *إذا كنت تعتقد أن هذا خطأ ولديك دليل، فيمكنك الاتصال بمالك الروبوت لاستئناف التعليق.* —◉ *التواصل مع مالكه:* wa.me/212645106267
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;FLIBU;;;\nFN:BOT\nORG:ＴＵＭＡＳ ＳＨＩＬＢＩ\nTITLE:\nitem1.TEL;waid=212645106267:212645106267\nitem1.X-ABLabel:ＴＵＭＡＳ ＳＨＩＬＢＩ\nX-WA-BIZ-DESCRIPTION:[❗] contact ＦＬＩＢＵ ＢＯＴ\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'ＦＬＩＢＵ ＢＯＴ', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ ⚠ ] Warning*', body: 'ＦＬＩＢＵ ＢＯＴ', thumbnail: imagen1, sourceUrl: 'https://instagram.com/flibu_gaming'}}}}, aa);
